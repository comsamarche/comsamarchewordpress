...
function maudtheme_scripts() {
	...
	if ( !is_admin() ) {
		...
		wp_enqueue_script( 'smoothstate-js', '//cdnjs.cloudflare.com/ajax/libs/smoothState.js/0.7.2/jquery.smoothState.min.js', array('jQuery'), date('Ymd'), true );
		wp_enqueue_script( 'smooth-state-initjs', get_template_directory_uri() . '/assets/js/main/smoothstate.js', array('jQuery', 'smoothstate-js'), date('Ymd'), true );
	}
